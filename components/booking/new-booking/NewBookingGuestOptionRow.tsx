...
export function NewBookingGuestOptionRow({
  ...
  return (
    <Pressable
      ...
      disabled={isDisabled}
      onPress={onPress}>
      <View className="h-12 w-12 items-center justify-center rounded-full bg-[#DCE9DD]">
        <Text className="text-[15px] font-semibold text-[#BDE111]">
          {getUserDisplayName(guest).slice(0, 1).toUpperCase()}
        </Text>
      </View>

      <View className="ml-3 flex-1">
        <Text className="text-[16px] font-medium text-[#171717]">{getUserDisplayName(guest)}</Text>
